...
detect_shell() {
  if [ -n "${ZSH_VERSION:-}" ]; then SHELL_NAME="zsh"; RC_FILE="$HOME/.zshrc"
  elif [ -n "${BASH_VERSION:-}" ]; then SHELL_NAME="bash"; RC_FILE="$HOME/.bashrc"
  else
...
ensure_fzf() {
  ...
  echo "fzf not found."
  read -rp "Install fzf now? [Y/n] " yn; yn="${yn:-Y}"
  case "$yn" in
    Y|y)
